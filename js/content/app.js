...
if(current_url.indexOf('parameter/rekening') != -1){
...
}else if(current_url.indexOf('/penatausahaan/skpd/tu/spm/up') != -1){
	var modal_spp = ''
		+'<div class="modal fade" id="mod-konfirmasi-program" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true" style="z-index: 99999">'
	        +'<div class="modal-dialog modal-xl" role="document">'
	            +'<div class="modal-content">'
	                +'<div class="modal-header bgpanel-theme" style="background: #8997bd;">'
	                    +'<h4 class="modal-title text-white" id="">Daftar Surat Permintaan Membayar (SPM)</h4>'
	                    +'<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="mdi mdi-close-circle"></i></span></button>'
	                +'</div>'
	                +'<div class="modal-body">'
	                	+'<div class="form-group row p-2">'
	                		+'<label for="mod-penandatangan" class="col-sm-3 col-form-label text-left font-weight-semibold border-bottom">Pilih Penandantangan</label>'
	                	...
	                	+'</div>'
	                  	+'<table class="table table-hover table-striped" id="konfirmasi-program">'
	                      	+'<thead>'
	                        	+'<tr style="background: #8997bd;">'
	                          		+'<th class="text-white"><input type="checkbox" id="modal_cek_all"></th>'
	                          		+'<th class="text-white" width="300">No SPM</th>'
	                          		+'<th class="text-white" width="300">SKPD</th>'
	                          		+'<th class="text-white" width="500">Uraian</th>'
	                        	+'</tr>'
	                      	+'</thead>'
	                ...
	                +'</div>'
	                +'<div class="modal-footer">'
	                    +'<button type="button" class="btn btn-success" id="singkronisasi-spm-modal">Proses</button>'
	                    +'<button type="button" class="btn btn-default" data-dismiss="modal">Tutup</button>'
	                +'</div>'
	            +'</div>'
	        +'</div>'
	...
	jQuery('body').append(modal_spp);
	var btn = ''
	+'<button type="button" class="btn btn-outline-success btn-sm" style="margin-left: 3px; float: right;" id="singkronisasi_data_spm_up">'
        +'<i class="fa fa-cloud-upload-alt fa-fw"></i> Singkronisasi Data SPM dari SIMDA'
    +'</button>';
    jQuery('h4.card-title').append(btn);
    jQuery('#singkronisasi_data_spm_up').on('click', function(){
    	show_loading();
    	window.tipe_spp_global = current_url.split('/').pop();
    	get_id_sub_unit_penatausahaan()
    	.then(function(sub_unit_fmis){
    ...
    jQuery('#singkronisasi-spm-modal').on('click', function(){
    	singkronisasi_spm_modal();
    });
}else if(current_url.indexOf('/penatausahaan/skpkd/bud/sp2d') != -1){
	var modal_spp = ''
		+'<div class="modal fade" id="mod-konfirmasi-program" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true" style="z-index: 99999">'
	        +'<div class="modal-dialog modal-xl" role="document">'
	            +'<div class="modal-content">'
	                +'<div class="modal-header bgpanel-theme" style="background: #8997bd;">'
	                    +'<h4 class="modal-title text-white" id="">Daftar Surat Perintah Pencairan Dana (SP2D)</h4>'
	                    +'<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="mdi mdi-close-circle"></i></span></button>'
	                +'</div>'
	                +'<div class="modal-body">'
	                	+'<div class="form-group row p-2">'
	                	...
	                	+'</div>'
	                  	+'<table class="table table-hover table-striped" id="konfirmasi-program">'
	                      	+'<thead>'
	                        	+'<tr style="background: #8997bd;">'
	                          		+'<th class="text-white"><input type="checkbox" id="modal_cek_all"></th>'
	                          		+'<th class="text-white" width="300">No SP2D</th>'
	                          		+'<th class="text-white" width="300">Penandantangan & Tanggal</th>'
	                          		+'<th class="text-white" width="500">Keterangan</th>'
	                        	+'</tr>'
	                      	+'</thead>'
	                      	+'<tbody></tbody>'
	                  	+'</table>'
	                +'</div>'
	                +'<div class="modal-footer">'
	                    +'<button type="button" class="btn btn-success" id="singkronisasi-sp2d-modal">Proses</button>'
	                    +'<button type="button" class="btn btn-default" data-dismiss="modal">Tutup</button>'
	                +'</div>'
	            +'</div>'
	        +'</div>'
    ...
    jQuery('#singkronisasi_data_sp2d').on('click', function(){
    	...
    	.then(function(sp2d){
    		var no_spm_all = [];
    		for(var i in sp2d){
    			if(sp2d[i].sp2d_no.indexOf('Tambah SP2D') != -1){
    				no_spm_all.push(sp2d[i].spm_no);
    			}
    		}
    		if(no_spm_all.length >= 1){
		    	var data = {
				    message:{
				        type: "get-url",
				        content: {
				...
				alert('Tidak ada SP2D yang perlu dibuat!');
				hide_loading();
			}
	    });
    });
    ...
    jQuery('#singkronisasi-sp2d-modal').on('click', function(){
    	singkronisasi_sp2d_modal();
    });
}else if(current_url.indexOf('/penatausahaan/skpd/sp2b') != -1){
	var modal_spp = ''
		+'<div class="modal fade" id="mod-konfirmasi-program" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true" style="z-index: 99999">'
	        +'<div class="modal-dialog modal-xl" role="document">'
	            +'<div class="modal-content">'
	                +'<div class="modal-header bgpanel-theme" style="background: #8997bd;">'
	                    +'<h4 class="modal-title text-white" id="">Daftar Surat Permintaan Pengakuan Belanja (SP2B) SIMDA</h4>'
	                    +'<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="mdi mdi-close-circle"></i></span></button>'
	                +'</div>'
	                +'<div class="modal-body">'
	                  	+'<table class="table table-hover table-striped" id="konfirmasi-program">'
	                      	+'<thead>'
	                        	+'<tr style="background: #8997bd;">'
	                          		+'<th class="text-white"><input type="checkbox" id="modal_cek_all"></th>'
	                          		+'<th class="text-white" width="300">No SP2B</th>'
	                          		+'<th class="text-white" width="300">SKPD</th>'
	                          		+'<th class="text-white" width="500">Uraian</th>'
	                        	+'</tr>'
	                      	+'</thead>'
	                ...
	                +'</div>'
	                +'<div class="modal-footer">'
	                    +'<button type="button" class="btn btn-success" id="singkronisasi-sp2b-modal">Proses</button>'
	                    +'<button type="button" class="btn btn-default" data-dismiss="modal">Tutup</button>'
	                +'</div>'
	            +'</div>'
	        +'</div>'
	...
	jQuery('body').append(modal_spp);
	var btn = ''
	+'<button type="button" class="btn btn-outline-success btn-sm" style="margin-left: 3px;" id="singkronisasi_data_sp2b">'
        +'<i class="fa fa-cloud-upload-alt fa-fw"></i> Singkronisasi Data SP2B dari SIMDA'
    +'</button>';
    jQuery('.btn[data-title="Tambah SP2B"]').parent().append(btn);
    jQuery('#singkronisasi_data_sp2b').on('click', function(){
    	show_loading();
    	get_id_sub_unit_penatausahaan()
    	.then(function(sub_unit_fmis){
	    	var data = {
    ...
    jQuery('#singkronisasi-sp2b-modal').on('click', function(){
    	singkronisasi_sp2b_modal();
    });
